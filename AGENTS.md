...
- Public GitHub `data/thesis.json`, `data/checks/latest.json`, `data/news/index.json`, and `data/news/items/*.json` are the canonical live records. `data/news.json` is a frozen migration fallback only. The Site fetches the split records at runtime; `dist/data/*` are deploy-time fallbacks only.
