...
- 2026-09-20 — Split canonical news/run-state storage into a small latest-check record, bounded news index, and one-file-per-item ledger so scheduled automation can make narrow validated writes. The legacy monolithic news record remains temporarily as a frozen compatibility fallback while the new reader path is verified.
